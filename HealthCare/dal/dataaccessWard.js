...
const ward = require(path.join(__dirname, './../models/ward'))(sequelize, Sequelize.DataTypes);
   ...
   async getData(req,resp){
    ...
    async getDataById(req,resp){
        ...
        if (validatedToken===false){
            return resp.status(401).send({
                ...
                req.decode = validatedToken;
                await sequelize.sync({ force: false });
                let rows = await ward.findOne({where:{WardNo:req.params.id}});
                if(rows===null){
    ...
    async postData(req,resp){
        ...
        async deleteData(req,resp){
            ...
            if (validatedToken===false){
                ...
                let depar = await ward.findOne({where:{WardNo: delid}});
                if(depar===null) {
                    return resp.status(200).send({message: `WardNo ${delid} does not exists`});
                }
                ward.destroy({
                where:{WardNo: delid}
                }).then((response)=>{
    ...
    async putData(req,resp){
        ...
        if (validatedToken===false){
            ...
            req.decode = validatedToken;
            await sequelize.sync({force:false});
            let wardExist = await ward.findOne({where:{WardNo: req.params.id}});
            if(wardExist===null) {
                return resp.status(409).send({message: `WardNo ${req.params.id} does not exists`});
            } 
            let war  = req.body;
            if(!validateObj.validateWard(war)){
            ...
            let rec = ward.update({
                WardNo:war.WardNo,
                WardType:war.WardType,
                
            },{
                where:{WardNo:req.params.id}
            }); 
            if(rec){
                return resp.status(200).send({
